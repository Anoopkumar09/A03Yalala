...
QUnit.test('Testing Addition function with four sets of inputs', function (assert) {
    assert.throws(function () { add(); }, new Error("only numbers are allowed"), 'Passing in array correctly raises an Error');
    assert.strictEqual(add(1,2), 3, 'All positive numbers');
    assert.strictEqual(add(1,-2), -1, 'Positive and negative numbers');
    assert.strictEqual(add(-1,-2), -3, 'All are negative numbers');
}); 

/*
QUnit.test('Testing Addition function with four sets of inputs', function (assert) {
    assert.throws(function () { div(); }, new Error("only numbers are allowed"), 'Passing in array correctly raises an Error');
    assert.strictEqual(div(1,0), error, 'not allowed because infinity');
    assert.strictEqual(div(1,-2), -1, 'negative numbers');
    assert.strictEqual(div(-1,-2), 0.5, 'All are positive integers');
}); 
*/
